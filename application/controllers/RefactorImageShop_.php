...
class RefactorImageShop extends AUTH_Controller {
	...
	public function uploadImageShop(){
		...
		$result = file_get_contents_curl(rtrim(trim($_REQUEST['urlshop']),"/")."/api-image.php",$_REQUEST["data"]);
		...
		//if ($result > 0) {			
			...
			$data = file_get_contents($_REQUEST["data"]['pImgen']);
			var_dump($data);
			file_put_contents($urlDMServer, $data);			
			$out['status'] = '';
