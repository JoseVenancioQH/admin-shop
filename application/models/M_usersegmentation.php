...
class M_usersegmentation extends CI_Model {
	...
	public function select_by_site($site) {
		$sql = "SELECT * FROM userpie_users WHERE website like '%{$site}%'";
	...
	public function select_by_name($name) {
		$this->db->select('*');
		$this->db->from('userpie_users');
		$this->db->where('username_clean',addslashes(sanitize($name)));
